...
GrpcJwt &GrpcJwt::init(const string        &key,
    ...
    std::call_once(m_once_, [&]() {
        instance_.reset(
            new GrpcJwt(key, secret_key, token_exp, refresh_token_exp));
    });
    return *instance_;
};

GrpcJwt &GrpcJwt::instance() {
...
UserSecurityInfo GrpcJwt::Secure(const grpc::ServerContext &context) const {
    ...
    if (! tokenStatus.isValid()) {
        throw SecurityException::Create(
            __FILE__, __FUNCTION__, __LINE__, tokenStatus.message());
    }
...
UserSecurityInfo GrpcJwt::Secure(const grpc::ServerContext &context,
    ...
    if (! session.IsValid(userSecurityInfo)) {
        throw SecurityException::Create(
            ...
            __FUNCTION__,
            __LINE__,
            "Session is not valid, re-authentication is needed");
    }
    return userSecurityInfo;
}

std::string GrpcJwt::CreateToken(const std::string &value) const {
    return Jwt::CreateToken(_key, value);
}

std::string GrpcJwt::CreateRefreshToken(const std::string &value) const {
    return Jwt::CreateRefreshToken(_key, value);
}

std::string GrpcJwt::UserFromToken(const std::string &token) const {
