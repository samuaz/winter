...
bool Date::operator<(const Date &rhs) const {
    ...
    if (rhs._year < _year) return false;
    if (_month < rhs._month) return true;
    if (rhs._month < _month) return false;
    if (_day < rhs._day) return true;
    if (rhs._day < _day) return false;
    if (_hour < rhs._hour) return true;
    if (rhs._hour < _hour) return false;
    if (_minute < rhs._minute) return true;
    if (rhs._minute < _minute) return false;
