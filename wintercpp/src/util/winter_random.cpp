...
unsigned int
winter::random::randomChar(const bool onlyABC, bool onlyHex) {
  ...
  if (onlyABC) {
    if (onlyHex) {
      return ((character >= 48 && character <= 57) || (character >= 65 && character <= 70)) ? character : randomChar(onlyABC, onlyHex);
    }
...
std::string
winter::random::generateHex(const unsigned int len) {
  std::stringstream ss;
...
std::string
winter::random::highResolutionClockHex() {
...
std::string
winter::random::uuid(bool dashed) {
  ...
  while (StreamSize(uuid) < 32) {
    uuid << (char)randomChar(true, true);
  }
...
std::string
winter::random::uuidDashed() {
  return uuid(true);
}

std::string
winter::random::uuidNoDashed() {
  return uuid(false);
}

std::string
winter::random::uuidRandom() {
