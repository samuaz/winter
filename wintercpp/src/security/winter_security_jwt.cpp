...
std::string
Jwt::createToken(const std::string &key, const std::string &value, std::chrono::seconds expire_time) const {
...
std::string
Jwt::CreateToken(const std::string &key, const std::string &value) const {
  return createToken(key, value, token_exp_);
}

std::string
Jwt::CreateRefreshToken(const std::string &key, const std::string &value) const {
  return createToken(key, value, refresh_token_exp_);
}

std::string
Jwt::ValueFromKey(const std::string &key, const std::string &token) const {
  using namespace jwt::params;
...
std::string
Jwt::Encode(const std::string &password) const {
  using namespace jwt::params;
  // Create JWT object
  jwt::jwt_object obj{algorithm(algorithm_), secret(secret_key_)};
...
jwt::jwt_object
Jwt::DecodeToken(const std::string &token) const {
  using namespace jwt::params;
...
TokenStatus
Jwt::ValidateToken(const std::string &token) const {
  using namespace jwt::params;

  if (token.empty()) {
    return TokenStatus(TokenStatusType::INVALID, "TOKEN IS MISSING, YOU NEED TO SEND YOUR TOKEN AS METADATA example: authorization: Bearer token");
  } else {
    try {
      ...
      return TokenStatus(TokenStatusType::INVALID, "Caught unknown exception");
    }
  }
}
