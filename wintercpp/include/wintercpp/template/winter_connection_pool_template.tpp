...
namespace winter::templates {
        ...
        if (pool_descriptor_.usePool()) {
            ...
            auto db_pool = [&]() {
                try {
                    ...
                    size_t actualConnections = available_connections_.size()
    ...
    template<typename TConnection>
    std::shared_ptr<TConnection> ConnectionPool<TConnection>::Conn() {
        if (pool_descriptor_.usePool()) {
            // CheckConnections need to work without block the thread because if
            // we block the thread all thread waiting will need to wait for the
            // first
            CheckConnections();
            std::scoped_lock lock(pool_mtx_);
            // if not available connections re do this function recursive it
            // will be taken by CheckConnections;
            if (available_connections_.empty()) { return Conn(); }
            ...
            return std::shared_ptr<TConnection>(
                ...
                std::bind(&ConnectionPool<TConnection>::ReleaseConn,
                          this,
                          std::placeholders::_1));
        } else {
    ...
    void ConnectionPool<TConnection>::ReleaseConn(TConnection *conn) {
        if (pool_descriptor_.usePool()) {
            // i prefer to create new connection instead of use old connection
            // because if connection get buggy new work will contain old
            // problems the add function block the thread;
            ...
            in_use_connections_.erase(
                std::remove_if(in_use_connections_.begin(),
