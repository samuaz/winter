...
class PreparedStatementField : public virtual Field<T>,
 ...
 public:
  ...
  PreparedStatementField(const std::string &name, const T &value) : Field<T>(name, value) {}

  PreparedStatementField(
      const std::string &name,
      const T &value,
      std::string custom_value) : Field<T>(name, value),
				  custom_value_(std::move(custom_value)) {}

  PreparedStatementField(const PreparedStatementField<T> &field) : Field<T>(field),
								   custom_value_(field.custom_value_) {}

  explicit PreparedStatementField(const PreparedStatementField<T> *field) : Field<T>(field),
									    custom_value_(field->custom_value_) {}
