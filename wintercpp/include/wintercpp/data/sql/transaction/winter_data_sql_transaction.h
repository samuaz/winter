...
class Transaction : public virtual ITransaction {
 public:
  /**
   * @brief
   * @param connection SQL connection type implementation, example: MysqlConnection, MariaDBConnection, PostgreSQLConnection
   * @param isolation_type The level of the SQL isolation level, if not passet the default isolation is TransactionIsolationType::DEFAULT, then will use the sql engine default
   * @param partial_commit Default FALSE, when true will commit even if one of the operations of the transaction fails, @warning this will break the atomicity
   * @return
   */
  ...
  /**
   * Execute the SQL statements lamda function, this lamda need to receive the transaction implementation as parameter and return T, in most cases T is represented by winter::data::response
   * in this funcion this transaction instances is delagated to the lambda function that then is the owner to execute the transaction
   *
   * @brief receive a lambda That Returns T and receive Transaction implementation as parameter, this is useful for example to receive and sql statement as a lambda
  ...
  /**
   * Execute the SQL the prepared statement from IStatement.prepared_statement() inside this transaction scope, delegating to the Exceute(const PreparedStatement &prepared_statement) method
   *
  ...
  /**
   * Execute the SQL the prepared statement from IStatement.prepared_statement() inside this transaction scope
   *
   * @brief receive a lambda That Returns T and receive Transaction implementation as parameter, this is useful for example to receive and sql statement as a lambda
   * @param IStatement interface implementation
   * @return TResponse
  ...
  /**
   * @brief return if transaction is configured to do partials commit, this means force commit the valid operations Although others have failed
   * @warning be careful if this is true will break the transaction atomicity
   * @return bool
   */
  ...
  /**
   * @brief returns the status of the transaction SUCCESS OR ERROR, if one of the operations failed, status is set to ERROR, only if all operations are successful status set to success
