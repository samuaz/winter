...
inline TEntityClass
FindById(const std::shared_ptr<Table> &table, TIDType id, TTransaction &transaction, std::function<TEntityClass(std::vector<TResultRow>)> entityGenerator) {
...
template <typename TEntityClass, typename TIDType, typename TTransaction, typename TResultRow>
std::vector<TEntityClass>
FindAll(const std::shared_ptr<Table> &table, TTransaction &transaction, std::function<std::vector<TEntityClass>(std::vector<TResultRow>)> entityGenerator) {
  return entityGenerator(Select() << From(table) >> transaction);
}

template <typename TTransaction>
inline int
Count(const std::shared_ptr<Table> &table, TTransaction &transaction) {
  auto res = Select("SELECT COUNT(*) AS Count FROM " + table->name()) >> transaction;
  if (res) {
...
template <typename TEntityClass, typename TIDType, typename TTransaction, typename TResultRow>
inline winter::data::Pageable<TEntityClass>
Page(
    ...
    std::function<std::vector<TEntityClass>(std::vector<TResultRow>)> entityGenerator) {
  return winter::data::Pageable<TEntityClass>(
      page_size,
      page_number,
      Count(table, transaction),
      entityGenerator(Select{"SELECT * FROM " + table->name() + " ORDER BY " + order_by + " LIMIT ? OFFSET ?"}.Value(page_size).Value(page_number) >> transaction));
}
...
SelectById(const std::shared_ptr<Table> &table, TIDType id) {
  ...
  switch (table->tableType()) {
	...
	select << Where(
	    Where::make_predicate(uuidTable->id(), Condition::EQ, id));
      }
      break;
    }
    default:
