...
DataTypeResult
ResultRow<TResultSet>::operator[](const winter::data::sql_impl::Column &column) const {
...
template <typename TResultSet>
DataTypeResult
ResultRow<TResultSet>::operator[](const std::string &column_name) const {
...
ResultRow<TResultSet>::Value(const winter::data::sql_impl::Column &column) const {
  auto column_name = column.name();
...
template <typename T>
winter::data::response::Response<T>
ResultRow<TResultSet>::Value(const std::string &column_name) const {
  return this[column_name].template as<T>();
}

template <typename TResultSet>
const Rows &
ResultRow<TResultSet>::rows() const {
  return rows_;
}

template <typename TResultSet>
const Rows &
ResultRow<TResultSet>::set_rows(const Rows &rows) {
  return rows_ = rows;
}
