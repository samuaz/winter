...
void DataBaseMigration<TConnectionType, TTransactionType>::execute() {
  ...
  transaction.template Execute<void>([&](TTransactionType & transaction) -> auto {
    Query(StatementType::kCreate, CreateMigrationTable().script) >> transaction;
    for (const auto &migration : Migrations()) {
      auto response = Select() << From(migration_table_)
		      ...
		      >> transaction;
      if (!response) {
	auto migrationResponse = (Query(StatementType::kCreate, migration.script) >> transaction);
	migrationResponse.template Then<void>(
	    [&](void) -> void {
	      Insert(migration_table_) << Values(
		  {Values::Add(migration_table_->name, migration.name),
		   Values::Add(migration_table_->script, migration.script),
		   Values::Add(
		       migration_table_->hash,
		       std::to_string(migration.Hash()))})
		  >> transaction;
	    ...
	    [&](void) -> void {
	      std::cerr << "EXITING,DB MIGRATION FAIL: "
			<< migration.name
			<< std::endl;
	      exit(EXIT_FAILURE);
	    });
...
template <typename TConnectionType, typename TTransactionType>
Migration
DataBaseMigration<TConnectionType, TTransactionType>::CreateMigrationTable()
    const {
  return Migration(
      "db_migrations",
      R"(
